-- =====================================================
-- Comprehensive Metrics Population Script (Corrected)
-- Based on Tenable's metrics and cybersecurity best practices
-- Using correct enum values: counter, gauge for type and security for category
-- =====================================================

-- Clear existing custom metrics (keep the original ones)
DELETE FROM metrics WHERE source IN ('database', 'calculated');
...
('systems_by_impact_high', 'Number of high impact systems', 'counter', 'security', (SELECT COUNT(*) FROM systems WHERE impact_level = 'High'), 'systems', '{"impact_level": "High"}', 'database', 'daily', NOW(), '{"table": "systems", "filter": "impact_level=High"}'),

('systems_by_impact_moderate', 'Number of moderate impact systems', 'counter', 'security', (SELECT COUNT(*) FROM systems WHERE impact_level = 'Moderate'), 'systems', '{"impact_level": "Moderate"}', 'database', 'daily', NOW(), '{"table": "systems", "filter": "impact_level=Moderate"}'),

('systems_by_impact_low', 'Number of low impact systems', 'counter', 'security', (SELECT COUNT(*) FROM systems WHERE impact_level = 'Low'), 'systems', '{"impact_level": "Low"}', 'database', 'daily', NOW(), '{"table": "systems", "filter": "impact_level=Low"}'),

-- System Authorization Status
('systems_authorized', 'Number of authorized systems', 'counter', 'security', (SELECT COUNT(*) FROM systems WHERE authorization_status = 'Authorized'), 'systems', '{"authorization_status": "Authorized"}', 'database', 'daily', NOW(), '{"table": "systems", "filter": "authorization_status=Authorized"}'),

('systems_pending_authorization', 'Number of systems pending authorization', 'counter', 'security', (SELECT COUNT(*) FROM systems WHERE authorization_status = 'Pending'), 'systems', '{"authorization_status": "Pending"}', 'database', 'daily', NOW(), '{"table": "systems", "filter": "authorization_status=Pending"}'),

-- System Health Metrics
('systems_compliance_percentage', 'Percentage of systems in compliance', 'gauge', 'security', 
...
-- Asset Count Metrics
INSERT INTO metrics (name, description, type, category, value, unit, labels, source, aggregation_period, last_calculated, metadata) VALUES
('total_assets', 'Total number of assets in inventory', 'counter', 'security', (SELECT COUNT(*) FROM assets), 'assets', '{"scope": "all"}', 'database', 'daily', NOW(), '{"table": "assets", "query_type": "count"}'),

('assets_with_agent', 'Number of assets with Tenable agent installed', 'counter', 'security', (SELECT COUNT(*) FROM assets WHERE has_agent = true), 'assets', '{"agent_status": "installed"}', 'database', 'daily', NOW(), '{"table": "assets", "filter": "has_agent=true"}'),

('assets_without_agent', 'Number of assets without Tenable agent', 'counter', 'security', (SELECT COUNT(*) FROM assets WHERE has_agent = false OR has_agent IS NULL), 'assets', '{"agent_status": "not_installed"}', 'database', 'daily', NOW(), '{"table": "assets", "filter": "has_agent=false"}'),

('assets_with_plugin_results', 'Number of assets with vulnerability scan results', 'counter', 'security', (SELECT COUNT(*) FROM assets WHERE has_plugin_results = true), 'assets', '{"scan_status": "scanned"}', 'database', 'daily', NOW(), '{"table": "assets", "filter": "has_plugin_results=true"}'),

-- Asset Coverage Metrics (Tenable-style)
('asset_coverage_percentage', 'Percentage of assets with active scanning', 'gauge', 'security', 
...
('agent_deployment_percentage', 'Percentage of assets with Tenable agent deployed', 'gauge', 'security', 
...
('assets_seen_last_7_days', 'Assets seen in the last 7 days', 'counter', 'security', 
...
('assets_seen_last_30_days', 'Assets seen in the last 30 days', 'counter', 'security', 
...
('assets_stale', 'Assets not seen in over 30 days', 'counter', 'security', 
...
INSERT INTO metrics (name, description, type, category, value, unit, labels, source, aggregation_period, last_calculated, metadata) VALUES
('total_vulnerabilities_new', 'Total number of vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities), 'vulnerabilities', '{"scope": "all"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "query_type": "count"}'),

('vulnerabilities_critical_new', 'Number of critical severity vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities WHERE severity_name = 'Critical'), 'vulnerabilities', '{"severity": "Critical"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "filter": "severity_name=Critical"}'),

('vulnerabilities_high_new', 'Number of high severity vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities WHERE severity_name = 'High'), 'vulnerabilities', '{"severity": "High"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "filter": "severity_name=High"}'),

('vulnerabilities_medium_new', 'Number of medium severity vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities WHERE severity_name = 'Medium'), 'vulnerabilities', '{"severity": "Medium"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "filter": "severity_name=Medium"}'),

('vulnerabilities_low_new', 'Number of low severity vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities WHERE severity_name = 'Low'), 'vulnerabilities', '{"severity": "Low"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "filter": "severity_name=Low"}'),

-- Vulnerability State Metrics
('vulnerabilities_open_new', 'Number of open vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities WHERE state = 'Open'), 'vulnerabilities', '{"state": "Open"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "filter": "state=Open"}'),

('vulnerabilities_fixed_new', 'Number of fixed vulnerabilities', 'counter', 'security', (SELECT COUNT(*) FROM vulnerabilities WHERE state = 'Fixed'), 'vulnerabilities', '{"state": "Fixed"}', 'database', 'daily', NOW(), '{"table": "vulnerabilities", "filter": "state=Fixed"}'),

-- Critical Vulnerability Metrics (Tenable-style)
('critical_open_vulnerabilities_new', 'Number of open critical vulnerabilities', 'counter', 'security', 
...
('high_open_vulnerabilities_new', 'Number of open high severity vulnerabilities', 'counter', 'security', 
...
-- CVSS Score Metrics
('avg_cvss_score', 'Average CVSS v3 base score across all vulnerabilities', 'gauge', 'security', 
...
('avg_cvss_critical', 'Average CVSS score for critical vulnerabilities', 'gauge', 'security',
...
INSERT INTO metrics (name, description, type, category, value, unit, labels, source, aggregation_period, last_calculated, metadata) VALUES
('vulnerabilities_age_0_7_days', 'Vulnerabilities discovered in last 7 days', 'counter', 'security',
...
('vulnerabilities_age_8_30_days', 'Vulnerabilities discovered 8-30 days ago', 'counter', 'security',
...
('vulnerabilities_age_31_90_days', 'Vulnerabilities discovered 31-90 days ago', 'counter', 'security',
...
('vulnerabilities_age_over_90_days', 'Vulnerabilities discovered over 90 days ago', 'counter', 'security',
...
('cyber_exposure_score', 'Simulated Cyber Exposure Score (0-1000)', 'gauge', 'security',
  LEAST(1000, ROUND(
...
('avg_asset_exposure_score', 'Average Asset Exposure Score', 'gauge', 'security',
...
-- Remediation Metrics
('remediation_rate_7_days', 'Percentage of vulnerabilities fixed in last 7 days', 'gauge', 'security',
...
('remediation_rate_30_days', 'Percentage of vulnerabilities fixed in last 30 days', 'gauge', 'security',
...
('assessment_maturity_grade', 'Assessment Maturity Grade (A-F)', 'gauge', 'security',
  CASE
...
('remediation_maturity_grade', 'Remediation Maturity Grade (A-F)', 'gauge', 'security',
  CASE
...
INSERT INTO metrics (name, description, type, category, value, unit, labels, source, aggregation_period, last_calculated, metadata) VALUES
('total_patches', 'Total number of patches available', 'counter', 'security', (SELECT COUNT(*) FROM patches), 'patches', '{"scope": "all"}', 'database', 'daily', NOW(), '{"table": "patches", "query_type": "count"}'),

('patches_critical', 'Number of critical severity patches', 'counter', 'security', (SELECT COUNT(*) FROM patches WHERE severity = 'Critical'), 'patches', '{"severity": "Critical"}', 'database', 'daily', NOW(), '{"table": "patches", "filter": "severity=Critical"}'),

('patches_high', 'Number of high severity patches', 'counter', 'security', (SELECT COUNT(*) FROM patches WHERE severity = 'High'), 'patches', '{"severity": "High"}', 'database', 'daily', NOW(), '{"table": "patches", "filter": "severity=High"}'),

-- Patch Age Metrics
('patches_age_0_30_days', 'Patches released in last 30 days', 'counter', 'security',
...
('patches_age_over_30_days', 'Patches available for over 30 days', 'counter', 'security',
  (SELECT COUNT(*) FROM patches WHERE release_date < NOW() - INTERVAL '30 days'),
...
SELECT 'Metrics population completed successfully!' as status;
SELECT
  category,
